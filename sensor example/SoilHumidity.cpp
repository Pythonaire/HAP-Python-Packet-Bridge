...
int ReadBattery() {
 ...
 int SoilHumidity() {
  digitalWrite(HUMPOWER, HIGH);delay(100);
 ...
 void dht22(float dhtValues[]) {
  digitalWrite(DHTPOWER, HIGH);delay(10);
  dht.begin();
  sensor_t sensor;
  ...
  dht.temperature().getEvent(&event);
    ...
    } else { dhtValues[0] = event.temperature;}
    dht.humidity().getEvent(&event);
    if (isnan(event.relative_humidity)) {
    //Serial.println(F("Error reading humidity!"));
    } else { dhtValues[1] = event.relative_humidity;}
...
void setup() {
  ...
  pinMode(DHTPOWER, OUTPUT);pinMode(HUMPOWER, OUTPUT);pinMode(VBATPIN, OUTPUT);
